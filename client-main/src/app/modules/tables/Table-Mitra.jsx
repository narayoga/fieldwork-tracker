...
const TableMitra = () => {
  ...
  const getItem = async () => {
    ...
    const url = `${process.env.REACT_APP_API_URL}mitra/read/`;
  ...
  const filteredPost = Array.from(posts).filter((val) => {
    if (search === "") {
      ...
      return val = val.subcon.toLowerCase().includes(search.toLocaleLowerCase())
    }
  })
          ...
          <table id="kt_table_users" className="table table-rounded table-striped border gy-5 gs-7" role="table">
            ...
            <thead>
              <tr className='fw-bolder text-uppercase'>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Nama Subcon</th>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Jumlah Man Power</th>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Jumlah Jointer</th>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Jumlah Mandor</th>
              </tr>
            </thead>
