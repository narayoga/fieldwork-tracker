...
const TableAdmin = () => {
  ...
  const approve = (nama) => {
    const body = { username: nama }
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const url = `${process.env.REACT_APP_API_URL}admin/approve/`;
  ...
  const keysFilter = ['username', 'handphone', 'role']
    ...
    <KTCard>
      ...
      <div className='card-body py-4'>
        ...
        <div className='table-responsive'>
          {/* begin::Table */}
          <table id="kt_table_users" className="table table-rounded table-striped border gy-4 gs-7" role="table">
            {/* begin::Table head */}
            <thead>
              <tr className='fw-bolder text-uppercase'>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Username</th>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>No. Telepon</th>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Role</th>
                <th className='text-center' role={"columnheader"} style={sto_style(5)}>Option</th>
              </tr>
            </thead>
            <tbody className='text-center text-gray-600 fw-bold' role={"rowgroup"}>
              ...
              {Array.from(currentPosts).map((item, i) => {
                return (
                  <tr key={i} role={"row"}>
                    <td className='text-center fw-normal' style={{ verticalAlign: "middle" }} role={"cell"}>
                      {item.username}
                    </td>
                    <td className='text-center fw-normal' style={{ verticalAlign: "middle" }} role={"cell"}>
                      {item.handphone}
                    ...
                    <td className='d-flex justify-content-center' role={"cell"} >
                      <div className='menu-item px-3'>
                        <button onClick={(e) => approve(item.username)} style={{ padding: "8px 10px" }} className='btn  btn-active-light-primary btn-sm position-static'>
