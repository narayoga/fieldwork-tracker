...
class ThemeMode {
  ...
  public getSystemMode = (): Mode => {
    return window.matchMedia('(prefers-color-scheme: light)') ? 'light' : 'light'
  }
    ...
    this.menu
      ...
      ?.forEach((item: HTMLElement) => {
        item.addEventListener('click', (e) => {
          ...
          const mode = menuMode === 'light' ? this.getSystemMode() : menuMode
  ...
  public flipImages = () => {
    document.querySelectorAll<HTMLElement>('[data-kt-img-dark]')?.forEach((item: HTMLElement) => {
      if (item.tagName === 'IMG') {
        if (this.getMode() === 'light' && item.hasAttribute('data-kt-img-light')) {
